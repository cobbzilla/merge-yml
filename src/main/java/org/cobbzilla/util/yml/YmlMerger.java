...
public class YmlMerger
{
    ...
    private final Map<String, Object> variablesToReplace = new HashMap<String, Object>();
    ...
    public YmlMerger(Map<String, String> variablesToReplace)
    {
        if (variablesToReplace != null) init(variablesToReplace);
    }

    private void init(Map<String, String> env)
    {
        for (String varname : env.keySet()) {
            variablesToReplace.put(varname, env.get(varname));
        }
    }
    ...
    public Map<String, Object> merge(String[] files) throws IOException
    {
        ...
        for (String file : files) {
            ...
            try {
                ...
                DEFAULT_MUSTACHE_FACTORY.compile(new StringReader(entireFile), "yaml-merge-" + System.currentTimeMillis()).execute(writer, variablesToReplace);
